...
export async function main(ns) {
    ...
    for(var i =0; i< hostNames.length; i++){
        ...
        if(ns.getServerMoneyAvailable(hostNames[i])<=0 || !ns.hasRootAccess(hostNames[i]) || hostNames[i] == "home" || ns.getServerRequiredHackingLevel(hostNames[i])> ns.getHackingLevel() || hackTime > 60){
            continue;
        }
        else{
    ...
    for(var k=0;k<servsAttacking;k++){
        if(k>prio.length-1){
            break;
        }
        await ns.sleep(100);
